...
let
  ...
  # Determine wallpaper path based on configuration
  wallpaperPath =
  ...
  # Function to substitute all placeholders
  substituteConfig = text:
    let
      ...
      allPlaceholders = colorPlaceholders ++ configPlaceholders;
      allValues = colorValues ++ configValues;
    in
    builtins.replaceStrings allPlaceholders allValues text;

  # Read template and process it
  configTemplate = builtins.readFile ./hyprlock.conf.template;
  processedConfig = substituteConfig configTemplate;
