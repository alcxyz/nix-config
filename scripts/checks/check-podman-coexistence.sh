...
for container in "$name-server" "$name-storage" "$name-peer" "$name-limits" "$name-bind" "$name-web-1"; do
...
compose() {
  timeout --foreground 90s env -u DOCKER_CONTEXT -u DOCKER_TLS_VERIFY -u DOCKER_CERT_PATH \
    ...
    docker compose --project-name "$name" --file "$work/compose.yaml" "$@"
}
cleanup() {
...
address=$(compose port web 8080)
[[ $(curl --noproxy '*' --fail --silent --show-error --retry 10 --retry-connrefused --retry-delay 1 --max-time 5 "http://$address") == podman-canary ]]
echo 'PASS: Docker Compose against isolated Podman API, including stop/start'
