...
# Keep publication authentication out of argv and repository files. Scope it
# to this repository so consumer input fetches keep using source-read access.
auth_config_index=${GIT_CONFIG_COUNT:-0}
if [[ ! "$auth_config_index" =~ ^[0-9]+$ ]]; then
  echo "Invalid inherited Git configuration count" >&2
  exit 1
fi
export "GIT_CONFIG_KEY_${auth_config_index}=http.${FORGEJO_URL}/${FORGEJO_OWNER}/${FORGEJO_REPO}.git.extraheader"
export "GIT_CONFIG_VALUE_${auth_config_index}=AUTHORIZATION: basic ${auth_header}"
export GIT_CONFIG_COUNT=$((auth_config_index + 1))
unset auth_header
