...
in {
  ...
  in
    ...
    assert lib.all (guard: guard.wantedBy == []) guards;
    assert lib.all (guard: !(guard.environment ? RUNNER_CONTAINER_LABEL)) guards;
    ...
    assert lib.all (guard: guard.environment.TRANSITION_TIMEOUT_SECONDS == "120") guards;
