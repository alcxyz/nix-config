...
{pkgs}: let
  ...
  actionScript = pkgs.writeText "podman-action.sh" ''
    ...
    if docker buildx create --name action-default --driver docker-container \
      ...
      --driver-opt cgroup-parent=/podmancanary.slice/podman-ci-api.service \
...
in
  pkgs.testers.runNixOSTest {
    name = "forgejo-rootless-podman-executor-paths";
    nodes.machine = {lib, ...}: {
    ...
    testScript = ''
      ...
      machine.succeed("systemd-run --unit=podman-qualification-runner --uid=podman-ci-runner --gid=podman-ci --working-directory=/run/podman-qualification --setenv=HOME=/var/lib/podman-ci-runner --setenv=DOCKER_HOST=unix:///run/podman-ci.sock ${pkgs.forgejo-runner}/bin/forgejo-runner exec --no-recurse --container-daemon-socket=unix:///run/podman-ci.sock --workflows .forgejo/workflows/qualification.yml --image runner-fixture:local push")
      ...
      machine.succeed("systemctl kill --kill-whom=all --signal=KILL podman-qualification-runner.service")
