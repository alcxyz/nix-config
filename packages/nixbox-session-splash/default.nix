...
stdenvNoCC.mkDerivation {
  ...
  installPhase = ''
    ...
    makeWrapper ${coreutils}/bin/timeout "$out/bin/nixbox-session-splash" \
      --run 'mode="''${1:-startup}"; case "$mode" in startup|boot-preview|shutdown|reboot) export NIXBOX_SPLASH_MODE="$mode"; if [ "$#" -gt 0 ]; then shift; fi ;; *) echo "usage: nixbox-session-splash [startup|boot-preview|shutdown|reboot]" >&2; exit 2 ;; esac' \
